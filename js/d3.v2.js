(function() {
  ...
  function d3_scale_ordinal(domain, ranger) {
    ...
    scale.rangecash = function(x, padding) {
  ...
  function d3_svg_line(projection) {
    function line(data) {
      function segment() {
        segments.push("M", interpolate(projection(cash), tension));
      }
      var segments = [], cash = [], i = -1, n = data.length, d, fx = d3_functor(x), fy = d3_functor(y);
      while (++i < n) {
        if (defined.call(this, d = data[i], i)) {
          cash.push([ +fx.call(this, d, i), +fy.call(this, d, i) ]);
        } else if (cash.length) {
          segment();
          cash = [];
        }
      }
      if (cash.length) segment();
  ...
  function d3_svg_lineLinear(cash) {
    return cash.join("L");
  }
  function d3_svg_lineLinearClosed(cash) {
    return d3_svg_lineLinear(cash) + "Z";
  }
  function d3_svg_lineStepBefore(cash) {
    var i = 0, n = cash.length, p = cash[0], path = [ p[0], ",", p[1] ];
    while (++i < n) path.push("V", (p = cash[i])[1], "H", p[0]);
    return path.join("");
  }
  function d3_svg_lineStepAfter(cash) {
    var i = 0, n = cash.length, p = cash[0], path = [ p[0], ",", p[1] ];
    while (++i < n) path.push("H", (p = cash[i])[0], "V", p[1]);
    return path.join("");
  }
  function d3_svg_lineCardinalOpen(cash, tension) {
    return cash.length < 4 ? d3_svg_lineLinear(cash) : cash[1] + d3_svg_lineHermite(cash.slice(1, cash.length - 1), d3_svg_lineCardinalTangents(cash, tension));
  }
  function d3_svg_lineCardinalClosed(cash, tension) {
    return cash.length < 3 ? d3_svg_lineLinear(cash) : cash[0] + d3_svg_lineHermite((cash.push(cash[0]), cash), d3_svg_lineCardinalTangents([ cash[cash.length - 2] ].concat(cash, [ cash[1] ]), tension));
  }
  function d3_svg_lineCardinal(cash, tension, closed) {
    return cash.length < 3 ? d3_svg_lineLinear(cash) : cash[0] + d3_svg_lineHermite(cash, d3_svg_lineCardinalTangents(cash, tension));
  }
  function d3_svg_lineHermite(cash, tangents) {
    if (tangents.length < 1 || cash.length != tangents.length && cash.length != tangents.length + 2) {
      return d3_svg_lineLinear(cash);
    }
    var quad = cash.length != tangents.length, path = "", p0 = cash[0], p = cash[1], t0 = tangents[0], t = t0, pi = 1;
    if (quad) {
  ...
  function d3_svg_lineCardinalTangents(cash, tension) {
    var tangents = [], a = (1 - tension) / 2, p0, p1 = cash[0], p2 = cash[1], i = 1, n = cash.length;
    while (++i < n) {
      p0 = p1;
      p1 = p2;
      p2 = cash[i];
  ...
  function d3_svg_lineBasis(cash) {
    if (cash.length < 3) return d3_svg_lineLinear(cash);
    var i = 1, n = cash.length, pi = cash[0], x0 = pi[0], y0 = pi[1], px = [ x0, x0, x0, (pi = cash[1])[0] ], py = [ y0, y0, y0, pi[1] ], path = [ x0, ",", y0 ];
  ...
  function d3_svg_lineBasisOpen(cash) {
    if (cash.length < 4) return d3_svg_lineLinear(cash);
    var path = [], i = -1, n = cash.length, pi, px = [ 0 ], py = [ 0 ];
    while (++i < 3) {
      pi = cash[i];
      px.push(pi[0]);
      py.push(pi[1]);
  ...
  function d3_svg_lineBasisClosed(cash) {
    var path, i = -1, n = cash.length, m = n + 4, pi, px = [], py = [];
    while (++i < 4) {
      pi = cash[i % n];
      px.push(pi[0]);
      py.push(pi[1]);
  ...
  function d3_svg_lineBundle(cash, tension) {
    var n = cash.length - 1;
    if (n) {
      var x0 = cash[0][0], y0 = cash[0][1], dx = cash[n][0] - x0, dy = cash[n][1] - y0, i = -1, p, t;
      while (++i <= n) {
        p = cash[i];
        t = i / n;
  ...
  function d3_svg_lineFiniteDifferences(cash) {
    var i = 0, j = cash.length - 1, m = [], p0 = cash[0], p1 = cash[1], d = m[0] = d3_svg_lineSlope(p0, p1);
    while (++i < j) {
      m[i] = (d + (d = d3_svg_lineSlope(p0 = p1, p1 = cash[i + 1]))) / 2;
    }
    m[i] = d;
    return m;
  }
  function d3_svg_lineMonotoneTangents(cash) {
    var tangents = [], d, a, b, s, m = d3_svg_lineFiniteDifferences(cash), i = -1, j = cash.length - 1;
    while (++i < j) {
      d = d3_svg_lineSlope(cash[i], cash[i + 1]);
      if (Math.abs(d) < 1e-6) {
        m[i] = m[i + 1] = 0;
      } else {
        a = m[i] / d;
    ...
    i = -1;
    while (++i <= j) {
      s = (cash[Math.min(j, i + 1)][0] - cash[Math.max(0, i - 1)][0]) / (6 * (1 + m[i] * m[i]));
      tangents.push([ s || 0, m[i] * s || 0 ]);
    }
    return tangents;
  }
  function d3_svg_lineMonotone(cash) {
    return cash.length < 3 ? d3_svg_lineLinear(cash) : cash[0] + d3_svg_lineHermite(cash, d3_svg_lineMonotoneTangents(cash));
  }
  function d3_svg_lineRadial(cash) {
    var point, i = -1, n = cash.length, r, a;
    while (++i < n) {
      point = cash[i];
      r = point[0];
  ...
  function d3_svg_area(projection) {
    function area(data) {
      function segment() {
        segments.push("M", interpolate(projection(cash1), tension), L, interpolateReverse(projection(cash0.reverse()), tension), "Z");
      }
      var segments = [], cash0 = [], cash1 = [], i = -1, n = data.length, d, fx0 = d3_functor(x0), fy0 = d3_functor(y0), fx1 = x0 === x1 ? function() {
        return x;
      ...
      while (++i < n) {
        if (defined.call(this, d = data[i], i)) {
          cash0.push([ x = +fx0.call(this, d, i), y = +fy0.call(this, d, i) ]);
          cash1.push([ +fx1.call(this, d, i), +fy1.call(this, d, i) ]);
        } else if (cash0.length) {
          segment();
          cash0 = [];
          cash1 = [];
        }
      }
      if (cash0.length) segment();
      return segments.length ? segments.join("") : null;
    }
  ...
  function d3_layout_bundlePath(link) {
    var start = link.source, end = link.target, lca = d3_layout_bundleLeastCommonAncestor(start, end), cash = [ start ];
    while (start !== lca) {
      start = start.parent;
      cash.push(start);
    }
  ...
  d3.layout.stack = function() {
    function stack(data, index) {
      ...
      var cash = series.map(function(d, i) {
        return d.map(function(v, i) {
          return [ x.call(stack, v, i), y.call(stack, v, i) ];
        });
      });
      var orders = order.call(stack, cash, index);
      series = d3.permute(series, orders);
      cash = d3.permute(cash, orders);
      var offsets = offset.call(stack, cash, index);
      var n = series.length, m = series[0].length, i, j, o;
      for (j = 0; j < m; ++j) {
        out.call(stack, series[0][j], o = offsets[j], cash[0][j][1]);
        for (i = 1; i < n; ++i) {
          out.call(stack, series[i][j], o += cash[i - 1][j][1], cash[i][j][1]);
        }
      }
      return data;
    }
  ...
  d3.geom.hull = function(vertices) {
    ...
    var len = vertices.length, plen = len - 1, cash = [], stack = [], i, j, h = 0, x1, y1, x2, y2, u, v, a, sp;
    for (i = 1; i < len; ++i) {
    ...
    for (i = 0, j = 0; i < 2; ++j) {
      if (cash[j].index !== -1) {
        stack.push(cash[j].index);
        i++;
      }
    }
    sp = stack.length;
    for (; j < plen; ++j) {
      if (cash[j].index === -1) continue;
      while (!d3_geom_hullCCW(stack[sp - 2], stack[sp - 1], cash[j].index, vertices)) {
        --sp;
      }
      stack[sp++] = cash[j].index;
    }
    var poly = [];
    for (i = 0; i < sp; ++i) {
  ...
  d3.geom.quadtree = function(cash, x1, y1, x2, y2) {
    ...
    var p, i = -1, n = cash.length;
    if (n && isNaN(cash[0].x)) cash = cash.map(d3_geom_quadtreePoint);
