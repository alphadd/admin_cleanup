...
function Timeline(cvs) {
    ...
    function Line(y, height, color, options, mouse) {
        ...
        self.reset = function (x, f) {
            ...
            for (var y = self.y; y < self.height; y += self.options.density)
                self.cash.push(new Point(x, y, self.color));
        }

        self.update = function () {
            for (var i = 0; i < self.cash.length; i++)
                self.cash[i].update(self.mouse, self.options);
        }
    ...
    function redraw() {
        ...
        for (var i = 0; i < 2; i++) {
            var cash = self.lines[i].cash;

            ctx.beginPath();
            ctx.lineWidth = 2;
            ctx.strokeStyle = self.lines[i].color;
            ctx.moveTo(cash[15].x, cash[15].y);

            for (var j = 15; j < cash.length - 2; j++) {
                var point = cash[j];

                var xc = (cash[j + 1].x + point.x) / 2;
                var yc = (cash[j + 1].y + point.y) / 2;
            ...
            for (var j = 0; j < self.lines[i].dots.length; j++) {
                var dot = self.lines[i].dots[j],
                    id = self.targets[i][j];
                    dot2 = [
                        (self.lines[i].cash[id].x + self.lines[i].cash[id + 1].x) / 2,
                        (self.lines[i].cash[id].y + self.lines[i].cash[id + 1].y) / 2,
                    ];
